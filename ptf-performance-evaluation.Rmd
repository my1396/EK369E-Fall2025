...
- **Relationship**: 
  ...
  $$\red{z_t = \ln (1+r_t)} \Longleftrightarrow e^{z_t}=1+r_t$$
...
.pull-right[
**Strength of log returns:**
- When returns are small, log returns approximate simple returns well.
- Log returns are time-additive, making them convenient for multi-period analysis.
- Many financial models assume log-normality of prices, which corresponds to normality of log returns.
]
...
- For log returns, a nice property is that the log return are additive:
...
(a) Calculate the arithmetic average monthly return.  
...
$$[3\%, -2\%, 5\%, -4\%, 1\%, -1\%, 2\%, -3\%, 4\%, -2\%]$$
<ol type="a">
  <li>First, calculate the <strong>arithmetic mean return</strong> of Sofia’s portfolio over the ten months.</li>
